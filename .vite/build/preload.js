...
const api = {
  ...
  onFileContentReady: (callback) => {
    ipcRenderer.on(
      "file-content-ready",
      (event, content, displayFileName, fullPath) => callback(event, content, displayFileName, fullPath)
    );
  },
  ...
  onSaveStatus: (callback) => {
    ipcRenderer.on(
      "save-status",
      (event, status, fullPath, displayFileName) => callback(event, status, fullPath, displayFileName)
    );
  },
